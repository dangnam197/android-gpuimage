...
import jp.co.cyberagent.android.gpuimage.sample.adapater.CategoryAdapter
import java.util.*
import kotlin.collections.HashMap

object GPUImageFilterTools {
    private var hashMap:HashMap<Enum<FilterType>,GPUImageFilter> = hashMapOf()
    fun showDialog(
            context: Context,
            listener: (filter: GPUImageFilter,type : FilterType) -> Unit
    ) {
        val filters1 = FilterList().apply {
        ...
        builder.setItems(filters1.names.toTypedArray()) { _, item ->
            listener(createFilterForType(context, filters1.filters[item]),filters1.filters[item])
        }
        builder.create().show()
    }
    ...
    fun setRecyclerView(
        ...
        var categoryAdapter = CategoryAdapter(object : CategoryAdapter.Interaction {
            override fun onItemSelected(position: Int, item: FilterType) {
                listener(createFilterForType(context, item))
            }

        })
        categoryAdapter.submitList(filters1)
    ...
    private fun createFilterForType(context: Context, type: FilterType): GPUImageFilter {
        return when (type) {
            FilterType.CONTRAST -> GPUWobbleFilter()
            ...
            FilterType.TONE_CURVE -> GPUImageToneCurveFilter().apply {
                setFromCurveFileInputStream(context.resources.openRawResource(R.raw.namtest))
            }
...
//                )
                GPUImageMultiplyBlendFilter().apply {
...
//                )
//                GPUImageAlphaBlendFilter(0.2f).apply {
//                    bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher)
//                }
                GPUImageMultiplyBlendFilter().apply {
                    bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher)
            ...
            FilterType.LOOKUP_AMATORKA -> GPUImageLookupFilter().apply {
                bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.namlut)
            }
    ...
    class FilterAdjuster(filter: GPUImageFilter, filterType: FilterType?=null) {
        ...
        init {
            adjuster = when (filter) {
                ...
                is GPUImageToneCurveFilter -> CurveAdjuster(filter)
                is GPUImageLookupFilter -> Custom(filter)
                is GPUImageAlphaBlendFilter -> AlphalAdjus(filter)
                is GPUImageMultiplyBlendFilter -> MultiplyAdjus(filter)
                is GPUImageOverlayBlendFilter -> OvelayAdjus(filter)
//                is GPUImageVibranceFilter -> VibranceAdjuster(filter)
        ...
        private inner class OvelayAdjus(filter: GPUImageOverlayBlendFilter) : Adjuster<GPUImageOverlayBlendFilter>(filter) {
            override fun adjust(percentage: Int) {
                filter.setOverlay(range(percentage, 0f, 1f))
            }
        }
        private inner class MultiplyAdjus(filter: GPUImageMultiplyBlendFilter) : Adjuster<GPUImageMultiplyBlendFilter>(filter) {
            override fun adjust(percentage: Int) {
                filter.setMultiply(range(percentage, 0f, 1f))
            }
        }
        private inner class AlphalAdjus(filter: GPUImageAlphaBlendFilter) : Adjuster<GPUImageAlphaBlendFilter>(filter) {
            override fun adjust(percentage: Int) {
                filter.setMix(range(percentage, 0f, 1f))
            }
        }

        private inner class Custom(filter: GPUImageLookupFilter) :
        ...
        private inner class ContrastAdjuster(filter: GPUImageContrastFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setContrast(range(percentage, 0.0f, 1.0f))
            }
        }

        private inner class CurveAdjuster(filter: GPUImageToneCurveFilter) :
                Adjuster<GPUImageToneCurveFilter>(filter) {
            override fun adjust(percentage: Int) {
                filter.setRateFilter(range(percentage, 0.0f, 100.0f))
            }
        ...
        private inner class GammaAdjuster(filter: GPUImageGammaFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setGamma(range(percentage, 0.0f, 1.0f))
            }
        }
